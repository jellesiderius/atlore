...
import { serverT, withRequestLocale } from '$lib/i18n/server';

export const handle: Handle = ({ event, resolve }) =>
  withRequestLocale(event, async () => {
    ...
    if (!['GET', 'HEAD', 'OPTIONS'].includes(event.request.method)) {
      ...
      if (
        ...
        return new Response(serverT('server.invalidOrigin'), { status: 403 });
      }
    }
...
export const handleError: HandleServerError = ({ error, status, message, event }) =>
  withRequestLocale(event, () => {
    if (status >= 500) console.error(error);
    return { message: status >= 500 ? serverT('common.unknownError') : message };
  });
